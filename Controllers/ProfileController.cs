    ...
    public class ProfileController : BaseController
    {
        ...
        public async Task<IActionResult> Edit(EditProfileViewModel model)
        {
            ...
            if (user.UserType == UserType.Seller)
            {
                ...
                // ═══════════════════════════════════════════════════════════════════════════════
                // ✅ معالجة الروابط بشكل منفصل - كل رابط له حالته الخاصة
                // ═══════════════════════════════════════════════════════════════════════════════

                // الرابط الأول
                if (model.WebsiteUrl1 != user.WebsiteUrl1)
                {
                    // تحقق أن الرابط الجديد مختلف عن المعلق الحالي
                    if (model.WebsiteUrl1 != user.PendingWebsiteUrl1 || user.PendingUrl1Status != "Pending")
                    {
                        ...
                        urlsChangedCount++;
                        _logger.LogInformation("✅ الرابط 1 تم تعديله للبائع {UserId}", userId);
                    }
                }
                ...
                if (model.WebsiteUrl2 != user.WebsiteUrl2)
                {
                    if (model.WebsiteUrl2 != user.PendingWebsiteUrl2 || user.PendingUrl2Status != "Pending")
                    {
                        ...
                        urlsChangedCount++;
                        _logger.LogInformation("✅ الرابط 2 تم تعديله للبائع {UserId}", userId);
                    }
                }
                ...
                if (model.WebsiteUrl3 != user.WebsiteUrl3)
                {
                    if (model.WebsiteUrl3 != user.PendingWebsiteUrl3 || user.PendingUrl3Status != "Pending")
                    {
                        ...
                        urlsChangedCount++;
                        _logger.LogInformation("✅ الرابط 3 تم تعديله للبائع {UserId}", userId);
                    }
                }

                // تحديث HasPendingUrlChanges بناءً على الحالات الفردية
        ...
        /// ✅ محدث: يحفظ كل رابط بشكل منفصل مع حالته الخاصة
        /// </summary>
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateStore(UpdateStoreViewModel model)
        {
            ...
            {
                _logger.LogInformation("✅ البائع {UserId} قام بتعديل {Count} رابط/روابط في صفحة ManageStore.",
                    userId, urlsChangedCount);
            }
